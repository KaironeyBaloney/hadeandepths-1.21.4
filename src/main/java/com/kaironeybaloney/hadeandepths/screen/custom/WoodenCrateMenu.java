...
public class WoodenCrateMenu extends AbstractContainerMenu {
    public final WoodenCrateBlockEntity blockEntity;
    private final Level level;
    private final Container container;
    private final int containerRows = 3;
    public WoodenCrateMenu(int windowId, Inventory playerInv, FriendlyByteBuf buf) {
        this(windowId, playerInv, (WoodenCrateBlockEntity) playerInv.player.level().getBlockEntity(buf.readBlockPos()));

    }

    public WoodenCrateMenu(int windowId, Inventory playerInv, WoodenCrateBlockEntity be)
    {
        super(ModMenuTypes.WOODEN_CRATE_MENU.get(), windowId);
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, ModBlocks.WOODEN_CRATE.get());
    }
